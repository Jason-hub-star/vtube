...
## 2026-06-11 입 패치 이중 턱선(V) 사건 — 생성 셀의 턱선 스트로크 컷

- 증상 (주인님 보고): 입을 벌리면 턱에 V 모양 선. canvas/pixi 양 백엔드 캡처 비교로 렌더러 무관 = 소재 문제로 확정.
- 원인: 생성 입 시트 셀에 캐릭터 턱선 스트로크가 딸려 들어옴 + `extract_mouth_states.py`의 적응형 하단 페이드가 콘텐츠 최하단(rows.max())까지 불투명 유지 → 어두운 턱선이 "콘텐츠"로 분류되어 생존. 패치는 0.97 스케일·립 정렬이라 패치 턱선이 원본 턱선보다 위에 찍혀 **이중 턱선**.
- 수정: 입술 블록 아래로 분리(>10행 갭)된 "가늘고(≤fh/8) 어두운(<피부휘도 0.75)" 블록을 턱선으로 판정해 콘텐츠에서 제외 + 하단 페이드 소프트 길이를 턱선 위에서 끝나게 클램프 (ramp_1d에 soft_end 추가).
- 검증: wide/mid/closed 합성 진단에서 단일 턱선 확인, pixi 런타임 실측 V 소멸, mesh verify 5/5 PASS 유지.
- 교훈: **턱선 가드는 원본 좌표 기준만으로 부족 — 패치 내부에 복제되어 들어온 윤곽 스트로크도 걸러야 한다** (정렬 스케일 ≠ 1이면 반드시 이중선이 된다).
