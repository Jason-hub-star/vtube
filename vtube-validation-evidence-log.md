...
- 확정 레시피: ① See-through 640 분해(기하·레이어 분리) ② `reskin_seethrough_layers.py` 재스킨(픽셀 소유권 앞→뒤 배타 부여, 보이는 영역=원본 픽셀, 가린 영역=인페인팅, 안개 알파 90 제거) ③ 눈 영역은 분해 레이어 제외하고 진짜 마스터 참조 슬롯 생성 눈 삽입 ④ 숨은 키포즈 레이어 = 슬롯 생성. 결과물 `hybrid_true.png` 검수 가능 판정.
