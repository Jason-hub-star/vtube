#!/usr/bin/env python3
"""Build and run the strong20 Live2D model carousel player."""
...
SANDBOX_REPORT = ROOT / "experiments" / "live2d-strong-model-pattern-001" / "reports" / "strong20_runtime_probe_sandbox.json"
...
def main() -> int:
    ...
    build = subprocess.run(
        [sys.executable, str(ROOT / "scripts" / "build_live2d_model_carousel_player.py")],
        cwd=ROOT,
        check=True,
        text=True,
        capture_output=True,
    ...
    url = f"http://127.0.0.1:{args.port}/model_carousel.html"
    print(f"\nLive2D 모델 플레이어 실행 중: {url}")
