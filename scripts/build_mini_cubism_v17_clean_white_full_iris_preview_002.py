#!/usr/bin/env python3
"""Build Character 002 v17 with cleaner fixed eye white and fuller moving iris assets."""
...
DEFAULT_OUT = EXP / "model_edit_v17_clean_white_full_iris_preview/mini_cubism_diagnostic_project"
DEFAULT_REPORTS = EXP / "reports/model_edit_v17_clean_white_full_iris_preview"
...
def largest_component_mask(mask: Image.Image) -> Image.Image:
    pix = mask.load()
...
def iris_candidate_mask(open_eye: Image.Image, side: str) -> Image.Image:
    ...
    cy = y0 + 0.46 * height
    gate = ellipse_mask(tuple(CANVAS_SIZE), (cx - width * 0.31, cy - height * 0.43, cx + width * 0.31, cy + height * 0.43), 0)
    out = Image.new("L", tuple(CANVAS_SIZE), 0)
    src = open_eye.load()
    gate_px = gate.load()
    out_px = out.load()
    for y in range(max(0, y0), min(CANVAS_SIZE[1], y1)):
        for x in range(max(0, x0), min(CANVAS_SIZE[0], x1)):
            if gate_px[x, y] <= 0:
                continue
            r, g, b, a = src[x, y]
            if a < 24:
            ...
            # Catch the full purple iris/pupil/highlight area, not just the dark pupil.
            if value < 170 or sat > 0.16 or (b > r + 8 and g >= r - 18):
                out_px[x, y] = min(255, int(a * 1.4))
    comp = largest_component_mask(out)
    comp = comp.filter(ImageFilter.MaxFilter(9)).filter(ImageFilter.GaussianBlur(1.2))
    return comp


def make_clean_white(open_eye: Image.Image, iris_mask: Image.Image) -> Image.Image:
    white = open_eye.copy().convert("RGBA")
    src = white.load()
    mask = iris_mask.filter(ImageFilter.MaxFilter(23)).filter(ImageFilter.GaussianBlur(4.0))
    mask_px = mask.load()
    for y in range(CANVAS_SIZE[1]):
        for x in range(CANVAS_SIZE[0]):
            amount = mask_px[x, y] / 255
            if amount <= 0:
                continue
            r, g, b, a = src[x, y]
            if a <= 0:
    ...
    for y in range(CANVAS_SIZE[1]):
        for x in range(CANVAS_SIZE[0]):
            if iris_px[x, y] < 20:
                continue
            r, g, b, a = src[x, y]
            if a > 20 and max(r, g, b) < 95:
                out_px[x, y] = min(255, int(a * 1.35))
    mask = mask.filter(ImageFilter.MaxFilter(5)).filter(ImageFilter.GaussianBlur(0.8))
    pupil = Image.new("RGBA", tuple(CANVAS_SIZE), (22, 20, 30, 0))
    pupil.putalpha(mask)
    return pupil
    ...
    for y in range(CANVAS_SIZE[1]):
        for x in range(CANVAS_SIZE[0]):
            if iris_px[x, y] < 20:
                continue
            r, g, b, a = src[x, y]
            if a > 20 and min(r, g, b) > 190 and max(r, g, b) - min(r, g, b) < 45:
                out_px[x, y] = min(220, int(a * 1.1))
    mask = mask.filter(ImageFilter.MaxFilter(3)).filter(ImageFilter.GaussianBlur(0.4))
    hl = Image.new("RGBA", tuple(CANVAS_SIZE), (255, 255, 255, 0))
    hl.putalpha(mask)
    return hl


def rebuild_eye_assets(character: dict[str, Any], project: Path) -> dict[str, Any]:
    metrics: dict[str, Any] = {}
    for side in ["L", "R"]:
        open_eye = Image.open(project / "parts" / f"eye_{side}_open.png").convert("RGBA")
        iris_mask = iris_candidate_mask(open_eye, side)
        white = make_clean_white(open_eye, iris_mask)
        iris = apply_mask(open_eye, iris_mask)
        pupil = make_pupil(open_eye, iris_mask)
        highlight = make_highlight(open_eye, iris_mask)
        metrics[f"eye_{side}_white"] = save_part(character, project, f"eye_{side}_white", white, "v17_clean_fixed_eye_white")
        metrics[f"eye_{side}_iris"] = save_part(character, project, f"eye_{side}_iris", iris, "v17_fuller_moving_iris")
        metrics[f"eye_{side}_pupil"] = save_part(character, project, f"eye_{side}_pupil", pupil, "v17_fuller_moving_pupil")
        metrics[f"eye_{side}_highlight"] = save_part(character, project, f"eye_{side}_highlight", highlight, "v17_moving_highlight")
    return metrics
...
def main() -> int:
    ...
    rebuilt = rebuild_eye_assets(character, out_project)
    character["experiment_id"] = "cubism-v2-new-character-002/model_edit_v17_clean_white_full_iris_preview"
    character["generated_at"] = now()
    character["notes"] = character.get("notes", []) + [
        "v17 preserves v15 bindings: eye white stays fixed; iris, pupil, and highlight move together.",
        "v17 rebuilds eye_L/R_white with stronger baked-iris removal so fixed original-eye residue no longer fights the moving detail layers.",
        "v17 rebuilds eye_L/R_iris from a fuller central iris mask to avoid only a partial original-eye region moving.",
    ]
    ...
    report = {
        "schema_version": 1,
        "generated_at": now(),
        "status": "V17_CLEAN_WHITE_FULL_IRIS_READY_FOR_VALIDATION",
    ...
    report_path = reports / "mini_cubism_v17_clean_white_full_iris_report.json"
