#!/usr/bin/env python3
"""Capture the Character 002 EyeOpen 0.27 Mini Cubism success-pattern packet."""
...
EYE_VALUES = [
    ("eyeopen_1_00_open", 1.0, "Open reference"),
    ("eyeopen_0_50_half", 0.5, "Half keypose"),
    ("eyeopen_0_27_natural_close", 0.27, "Natural close threshold"),
    ("eyeopen_0_00_full_close", 0.0, "Technical full-close"),
]
...
def run_browser_capture(url: str, out_dir: Path) -> dict[str, Any]:
    ...
    result_path = out_dir / "eye_open_027_browser_result.json"
    config_path = out_dir / "eye_open_027_browser_config.json"
    script_path = out_dir / "eye_open_027_capture.cjs"
    config = {
        "url": url,
        "framesDir": str(frames_dir),
        "resultPath": str(result_path),
        "frames": [
            {"id": frame_id, "eyeOpen": value, "label": label}
            for frame_id, value, label in EYE_VALUES
        ],
    }
    ...
    if completed.returncode != 0:
        ...
        raise RuntimeError(f"eye-open capture failed\n{detail}\n{completed.stderr}")
...
def build_contact_sheet(frames: list[dict[str, Any]], out_path: Path) -> None:
    ...
    margin = 28
    header_h = 68
    sheet = Image.new("RGB", (margin * 2 + cell_w * len(frames), margin * 2 + header_h + cell_h), "#151719")
    draw = ImageDraw.Draw(sheet)
    title_font = font(24)
    label_font = font(18)
    small_font = font(14)
    draw.text((margin, margin), "Character 002 Mini Cubism EyeOpen 0.27 Success Packet", fill="#f1f3f4", font=title_font)
    draw.text(
        (margin, margin + 34),
        "Captured from v7 preview with ParamMouthOpenY=0; 0.27 is the natural-close review threshold.",
        fill="#b8c0c8",
        font=small_font,
    )
    for index, frame in enumerate(frames):
        ...
        full = fit_image(image, (cell_w - 24, 310))
        crop = fit_image(eye_crop(image), (cell_w - 24, 120))
        draw.rounded_rectangle((x + 6, y + 6, x + cell_w - 6, y + cell_h - 6), radius=8, fill="#202124", outline="#3a3f45")
        draw.text((x + 18, y + 18), f"EyeOpen {frame['eyeOpen']:.2f}", fill="#f1f3f4", font=label_font)
        draw.text((x + 18, y + 44), frame["label"], fill="#b8c0c8", font=small_font)
        sheet.paste(full, (x + 12, y + 76))
        draw.text((x + 18, y + 396), "eye close-up", fill="#b8c0c8", font=small_font)
        sheet.paste(crop, (x + 12, y + 418))
        active = active_eye_parts(frame)
        active_label = ", ".join(active)
        if len(active_label) > 45:
            active_label = active_label[:42] + "..."
        draw.text((x + 18, y + 558), active_label, fill="#d7dadc", font=small_font)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    sheet.save(out_path)


def active_eye_parts(frame: dict[str, Any]) -> list[str]:
    ...
    for part_id in [
        ...
        if float(part_opacity.get(part_id, 0)) >= 0.5:
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    for frame in report["frames"]:
        ...
        lines.append(
            f"| {frame['eyeOpen']:.2f} | {frame['label']} | `{active}` | "
            f"{frame['changed_ratio_vs_open']:.6f} | `{frame['screenshot']}` |"
        )
    lines.extend(
        [
            "",
...
def build_report(project: Path, out_dir: Path, browser_result: dict[str, Any]) -> dict[str, Any]:
    ...
    open_path = Path(frames[0]["screenshot"])
    for frame in frames:
        frame["active_eye_parts"] = active_eye_parts(frame)
        frame["changed_ratio_vs_open"] = changed_ratio(open_path, Path(frame["screenshot"]))
        frame["screenshot"] = str(Path(frame["screenshot"]))
    contact_sheet = out_dir / "eye_open_027_contact_sheet.png"
    ...
    return {
        ...
        "decision": {
            "natural_close_eye_open_value": 0.27,
            "technical_full_close_value": 0.0,
            "production_claim": "Mini Cubism diagnostic/keypose evidence only; not CMO3/MOC3 authoring success.",
        },
        "frames": frames,
    }
...
def main() -> int:
    ...
    try:
        ...
        report_path = out_dir / "eye_open_027_success_report.json"
        md_path = out_dir / "eye_open_027_success_report.md"
