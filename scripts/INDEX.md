...
| `bake_arap_blink_layers.py` ⭐ | 94 | ARAP 깜빡임 프레임을 리그용 inbetween 레이어로 굽는다 (런타임 무수정 통합). |
...
| `build_eye_smile_candidates.py` | 81 | EXPR-002 눈웃음 후보 비교 스트립 — ∩/∪ 곡선 후보를 실제 조립본에 렌더해 주인님이 고른다. |
...
| `run_arap_blink_experiment.py` | 250 | ARAP-EXP-001: 원본 픽셀만으로 눈꺼풀 감김(깜빡임)을 만드는 메시 워프 실험. |
