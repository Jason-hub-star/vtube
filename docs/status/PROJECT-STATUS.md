...
- **통합**: `integrate_angle_swap.py` 원본 통째 복사 → `rig_v0_angle_project`(58파트). ParamAngleX ±80, 중앙 라이브밴드 |X|≤14(표정 라이브)+외곽 작화밴드 16/32/48/64/80 하드 스왑. **런타임 코드 0변경**.
- **정렬 폴리시(A)**: 본체 정면 실루엣 합성 실측(머리 cx 1019·top 36·폭 510) → 작화 ×1.25 확대+머리앵커 일치(팝 제거), 작은 비연결 컴포넌트 제거(연두 유령 파편 제거).
- **실측**: 8065 헤드리스 캡처 9장 — 정면=라이브 표정(입 개폐 생존)·좌우 ±80=옆모습 회전·크로스페이드 더블렌더 0·머리 크기/위치 정면 일치. `reports/angle_swap_capture/`.
- **알려진 한계**: 작화는 머리+가슴 크롭이라 측면 회전 시 하체/치마 사라짐(버스트업 뷰엔 무방). 끄덕임(AngleY)·하이브리드는 다음 카드.
- **후속 (미검증)**: ① 끄덕임 상하 각도시트(AngleY) ② 하이브리드(작은각도 메시키폼+큰각도 작화스왑) ③ 트래킹 yaw 게인 ±80 확장(웹캠 풀턴 도달).
