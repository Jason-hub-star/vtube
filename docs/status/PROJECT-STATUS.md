...
Updated: 2026-06-15 (005 지피쨩 "코드 0줄 3D급" 빌드 — 머리XYZ입체+상체회전 자동 상속, 맥 MPS 분해, 입 3단 수정. 다음: 턱 수염=face_base 알파 가장자리 페더)
...
- 프로덕션 비전 (즉각성, 병목 분석): `docs/ref/PRODUCTION-VISION.md` (2026-06-15, 리허설 1분 38초 실증 + 게이트 병목 분석).
...
## 다음 세션 빠른 재개 — 005 지피쨩 인수인계 (2026-06-15)

**005 지피쨩 = 첫 "코드 0줄 3D급" 캐릭터.** 머리 XYZ 입체(head_z sx0.88/sy0.85)+상체회전
(torso_turn_warp sx0.96)을 빌더가 자동 내장. 맥 MPS로 분해(CUDA 불필요).
rig: `experiments/autorig-character-005/rig_v0_project`.

- ✅ **입 부품 깨짐 해소 (2026-06-15 후속)**: 이전 "단일 입 정상"은 오판 — 입 시트에 살구색 얼굴이
...
- **005 재빌드(분해 후)**: `build_autorig_rig_v0.py --reskin-manifest .../seethrough_true_reskinned/reskin_manifest.json
  --arap-eye-dir … --mouth-parts-dir … (위벨과 동일 인자) --out-dir .../rig_v0_project`. 또는 풀런
  `run_autorig_pipeline.py --reuse-decomposition <layers.json> --skip-gates`.
- **떠있을 수 있는 서버**(세션 종료 시 꺼짐): 8062 위벨 입·8063 위벨 드라이브·8066 005 드라이브
  (**`/drive` 경로!**)·8188 ComfyUI·8095 관제탑.
  - 005 드라이브: `python3 scripts/run_mini_cubism_webcam_drive.py --project experiments/autorig-character-005/rig_v0_project --port 8066` → `http://127.0.0.1:8066/drive`
- **대기 게이트(위벨)**: 입 풀런 `runs/autorig-character-004_20260613_193420` H2 WAITING.
