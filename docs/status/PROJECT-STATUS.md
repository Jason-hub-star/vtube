...
Updated: 2026-06-13 (입 MOUTH-LIP-PARTS·템플릿화 TEMPLATE-001·각도입체감 ANGLE-SWAP-002 본체통합 완료. 입 H2 판정이 다음 차례)
...
### ANGLE-SWAP-002 — 우향 미러 + 본체 실제 통합 (2026-06-13 실증)

좌향 검증 미니를 **본체 위벨 리그(50파트·표정·입)에 비파괴 통합**. 핵심 근거: rig.js `partOpacity`(L194)가 파트별 opacity 곡선을 **곱한다** → 라이브 50파트에 ParamAngleX 크로스페이드 곡선 1개씩 더해도 기존 표정/입 곡선과 충돌 없이 합성.

- **우향**: `mirror_angle_parts.py` 좌향 작화 수평 flip(무료, 캔버스중심 x=1024 앵커) → 우4장. 주인님 선택(단순 미러).
- **통합**: `integrate_angle_swap.py` 원본 통째 복사 → `rig_v0_angle_project`(56파트). ParamAngleX ±80. **라이브밴드 |X|≤26(±34 페이드)=본체 메시 변형 키폼 풀범위+표정 라이브**, 작화는 극단(±34~80, 40/60/80°)에만. **런타임 코드 0변경**.
- **정렬 폴리시(A)**: 본체 정면 실루엣 합성 실측(머리 cx 1019·top 36·폭 510) → 작화 ×1.25 확대+머리앵커 일치(팝 제거), 작은 비연결 컴포넌트 제거(연두 유령 파편 제거).
- **라이브밴드 ±26 복구(핵심)**: 초기 ±14는 변형 리그를 너무 일찍 죽여 "사진 스왑" 체감이 컸음. 본체 메시는 ±30까지 실제 변형하므로 라이브를 ±26까지 살림 → **평소 회전=살아있는 변형+입/눈/표정**(8065 캡처 L20+입벌림 생존 확증), 작화 사진은 ±40+ 극단에서만(실제 Live2D 한계와 동일 수준).
- **드라이브 연동**: `run_mini_cubism_webcam_drive.py --project rig_v0_angle_project --port 8066`. `drive.html` ParamAngleX 매핑을 프로젝트 max(±80) 하이브리드 이즈드 곡선으로(중앙 공식느낌+큰턴 가속, 경량 EMA). 단 웹캠 추적은 ~35° 넘으면 얼굴 놓침 — 풀옆모습은 슬라이더/극단증폭 영역.
- **3D 피벗 검토 결론(2026-06-13)**: PNG→3D 원클릭 리깅 툴 없음(검색 확증). 이미지→3D 메시(Hunyuan3D 3.5/Trellis 2/Tripo)는 정적 메시만+애니 얼굴 복원 약함+리그 없음. 진짜 3D는 VRoid+VRM 별도 파이프라인(2D 자동리깅 폐기). → **2D 유지 결정**(우리 방식=실제 Live2D 방식, 풀옆모습 한계도 동일).
- **알려진 한계**: 작화는 머리+가슴 크롭이라 ±40+ 측면 시 하체/치마 사라짐(버스트업 무방)·옆모습 작화는 정지(표정 없음). 끄덕임(AngleY)·옆모습 생성리깅은 다음 카드.
- **후속 (미검증)**: ① 끄덕임 상하 각도시트(AngleY) ② 옆모습 생성리깅(눈감음·입벌림 변형 생성 — 절차적 눈꺼풀은 PoC 실패: 연두 덩어리) ③ 하이브리드 미세조정.

## Next Actions

1. **004 입 H2 판정** — 8062 입 슬라이더로 미소 개폐 확인 → 합격 시 정비 코드(TEMPLATE-001)로 통합 풀런 → rig v2.0 박제. 잔여 후보: 치마 드레이프 ±9px(주인님 체감).
2. **ANGLE-SWAP 후속** — 우향 미러+본체 통합 완료(8065 실증). 다음 갈림길: 끄덕임 AngleY 시트 / 하이브리드 / 트래킹 yaw 게인 ±80. 입체 각도가 위벨뿐 아니라 전 캐릭터 공통 자산.
...
- **떠있을 수 있는 서버**(세션 종료 시 꺼짐 — 재기동 필요): 8062 입 리깅앱·8063/drive 웹캠·8064 각도스왑·8095 관제탑.
  ...
  - 각도(미니): `python3 scripts/mini_cubism_preview_server.py --project experiments/autorig-character-004/angle_swap_project --port 8064`
  - 각도(본체통합): `python3 scripts/mini_cubism_preview_server.py --project experiments/autorig-character-004/rig_v0_angle_project --port 8065`
