...
- 현행 계보 산출물은 `autorig-` 접두사 + 자체 버전 (예: rig v0 → v0.1 → v1). "v숫자" 단독 표기는 구계보(코덱스 시절 v1~v22) 인용일 때만 쓴다. v21=눈 성공패턴(수치 이식됨), v22=마지막 소재 스펙(키포즈 raw를 변형 가이드로 상속).
- **픽셀-가이드 원칙 (2026-06-11)**: 생성물은 "변형 가이드"로만 쓰고, 화면 픽셀은 항상 원본 레이어(또는 그 메시 변형)다. 예외는 원본에 물리적으로 존재하지 않는 내부(입 안)뿐.
- **표정 부위별 공식 (2026-06-11 주인님 합격, 상세는 evidence log "입 성공패턴")**: 눈=ARAP 워프(가림 동작) / 입=동일세션 4상태 스프라이트+밀착 페이드+턱선 가드(외형 변화 동작). 입에 워프 금지 — 닫힌 미소선은 변형해도 열린 입술이 되지 않는다 (경첩 턱).
...
7. **에러 삼키기 금지**: 조용한 `except: pass`/광역 `except Exception` 무시 금지 — 에러는 경계(파이프라인 스테이지, 서버 핸들러)에서만 처리하고, 스크립트 내부에서는 실패를 그대로 전파한다 (조용한 실패는 증거를 오염시킨다 — 바이브코딩 체크리스트 2026-06-11 채택).
...
3. `AUTORIG-PIPELINE-CLI-001` — P0–P6 원커맨드 CLI ✅ **완료 (2026-06-11)**: `scripts/run_autorig_pipeline.py` — 마스터+입시트 2장 입력, H1.5/H2 관제탑 게이트, 003 풀런 76분(순수 연산 ~22분) 실증
...
## 사용자 입력 설계 — AUTORIG-ONBOARD-001 (결정 2026-06-11, 구현은 PLAYER-001 이후)

**원리: 사용자 입력은 '스타일 소스'일 뿐, 마스터는 항상 우리 고정 템플릿이 재생성한다.**
MASTER-SPEC 조건 9개는 우리 사정이지 고객의 의무가 아니다 — 생성 단계가 포즈·배경·조건을 정규화한다.

```text
고객 입력 (선택칩 / 사진 업로드) → 스타일 레퍼런스로만 사용
  → 고정 프롬프트 스켈레톤이 마스터 재생성 (조건 1~9 하드코딩, 칩은 "캐릭터 묘사" 슬롯만)
  → P0 자동 검증 (validate_master_image — FAIL 사유를 사용자 친화 문구로 매핑)
  → 고객 H1 승인 ("이 모습 맞아요?" — 분해 20분 전 차단, 재생성 무한)
  → 원커맨드 풀런
```

- **프롬프트를 사용자에게 직접 주지 않는다** — 조건 스켈레톤이 깨진다. 칩(머리 길이/색/의상/분위기/액세서리)이 묘사 슬롯만 채운다.
- 사진 업로드 경로는 같은 깔때기에 합류: rembg 배경 제거 + `AUTORIG-ANCHOR-DETECT-001` 전처리 후 스타일 레퍼런스로.
- 사진 가이드라인은 보조 1줄("정면, 얼굴이 보이게")만 — 긴 가이드는 읽히지 않는다.
- 방어 4겹: 스켈레톤 고정 → P0 자동 검증 → H1 고객 승인 → 자동 방어 스테이지 (MASTER-SPEC 3.5절).

## 사용자 산출물 정의 (프로덕션, 2026-06-11)

- `.moc3`(VTube Studio용)는 Cubism Editor 독점 export라 제공 불가 — 우리 구조의 전제. 표준 모델 폴더 구성 참고: `reference-model-structure-001/reports/all57_runtime_metadata_extras.md`.
- 제공물: ① 웹 링크(브라우저 웹캠 버튜버) ② **OBS 브라우저 소스 URL** (투명배경 플레이어 — 방송 송출의 정석, 핵심 상품) ③ 다운로드 ZIP: `character.json + parts/*.png + meshes/*.json + mini_rig.json + player.html`(단독 플레이어, DOM-free core 기반) ④ 후순위: Inochi2D `.inp` export 어댑터.
- 선결 TODO: 플레이어 투명 배경 모드 — pixi 백엔드에 `?transparent=1` 배선 완료(backgroundAlpha 0 + 배경 스프라이트 숨김), `AUTORIG-PLAYER-001`에서 플레이어 연결만 남음.
