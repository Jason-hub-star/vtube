...
- `AUTORIG-RUNTIME-SPLIT-001`: `mini_cubism_app/src/app.js`(1,673줄 모놀리스)를 core(렌더/리그/물리)·ui·probe 모듈로 분할. 분할 후 T3 스모크(`run_mini_cubism_webcam_drive_smoke.py`)와 eye mode validator로 무회귀 검증. 이 분할 전에는 mini_cubism_app에 기능을 추가하지 않는다.

## 다음 작업 ID

1. `AUTORIG-TEMPLATE-SPEC-001` — 고정 슬롯 시트 레이아웃 스펙 + 생성 프롬프트 템플릿 (64-part를 슬롯에 매핑)
