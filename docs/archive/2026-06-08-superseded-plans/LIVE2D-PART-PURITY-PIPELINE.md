...
Apple Silicon 저해상도 compatibility smoke:

```bash
python3 scripts/setup_mps_compat_002.py
python3 scripts/run_mps_compat_matrix.py --case mps_512_safe --dry-run
```

실제 실행은 ComfyUI-See-through 서버가 떠 있을 때만 한다.

```bash
python3 scripts/run_mps_compat_matrix.py --case mps_512_safe
```

512에서 `*_layers.json`이 생성될 때만 640, 768로 확장한다. 512 실제 실행은 처음에
`layerdiffuse/vae.py:232`의 MPS unsupported sort/median path에서 실패했지만,
`scripts/patch_comfyui_seethrough_mps.py` 적용 후 `PASS_MPS_512_LAYERS`가 되었다. Mac 경로의
성공 기준은 production 품질이 아니라 `*_layers.json` 생성과 review app 연결이다.
자세한 비교 문서는:

```text
docs/ref/MAC-LAYER-DECOMP-OPTIONS.md
```
...
Mac MPS compatibility 결과를 정규화할 때는 experiment를 명시한다.

```bash
python3 scripts/normalize_seethrough_outputs.py \
  --experiment-id see-through-mps-compat-002 \
  --experiment-dir experiments/see-through-mps-compat-002 \
  --layers-json <ComfyUI output *_layers.json>
```
...
- `Mac MPS 후보` 탭은 `see-through-mps-compat-002/layer_manifest.json`이 생긴 뒤 표시된다.
- 현재 `Mac MPS 후보`는 29개 생성되었고, 전부 human review 전까지 PSD 제외 상태다.
