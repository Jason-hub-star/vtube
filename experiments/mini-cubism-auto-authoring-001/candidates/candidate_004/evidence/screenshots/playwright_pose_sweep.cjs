...
async function setPose(page, parameters) {
  await page.evaluate((values) => {
    ...
    for (const row of rows) {
      ...
      const spec = project?.parameters?.find((item) => item.id === paramId);
      const nextValue = Object.prototype.hasOwnProperty.call(values, paramId) ? values[paramId] : spec?.default;
